...
#    def svd(self, shape, ncomps = 15, dtype = float):
#        # Assumes rmatvec is well defined (e.g. self adjoint).
#        # I found surprisingly that scipy SVD is faster than eigsh.
#        from scipy.linalg.interpolative import svd
#        op_sp = self.to_scipy(shape = shape, dtype = dtype)
#
#        pc_funs, singular_vals, _ = svd(op_sp, ncomps)
#        pc_funs = pc_funs.T.reshape((-1, *shape)) # Shape [ncomps, *shape].
#        return pc_funs, singular_vals
