...
#    angles = torch.randint(0, 10, size = (inp.shape[0],)).float() * np.pi / 5.

    angles = torch.rand(size=(inp.shape[0],)) * np.pi * 2
...
def accuracy(X, Y):
    ...
    cnd1 = torch.sum(torch.abs(X[:, -1, 0] - Y[:, -1, 0]) < 1e-2) # Fixation. UNUSED FOR NOw.
    cnd2 = (torch.abs(X[:, -1, 1] - Y[:, -1, 1]) < 1e-2) # Stim1
    cnd3 = (torch.abs(X[:, -1, 2] - Y[:, -1, 2]) < 1e-2) # Stim2
    return torch.mean(torch.logical_and(cnd2, cnd3).float())
